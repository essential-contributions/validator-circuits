...
impl Circuit for ValidatorParticipationCircuit {
    ...
    fn wrappable_example_proof(&self) -> Option<Self::Proof> {
        ...
        let stakes = [64];
        let rounds = [32];
        let (validators_tree, accounts_tree, validators_state_proof) =
            example_validators_state(&validators_state_circuit, &accounts, &validator_indexes, &stakes);
        let (validator_epochs_tree, participation_rounds_tree, participation_state_proof) = example_participation_state(
...
fn example_participation_state(
    ...
    let mut previous_proof: Option<ParticipationStateProof> = Some(load_or_create_init_proof::<
        ParticipationStateCircuit,
