...
pub fn generate_partial_witness(
    ...
    if data.agg2_data.len() != AGGREGATION_STAGE3_SIZE {
        return Err(anyhow!(
            "Must include {} datas in attestation aggregation third pass",
            AGGREGATION_STAGE3_SIZE
        ));
    }
    ...
    if dummy_proof.is_none() {
        return Err(anyhow!(
            "Must include at least one valid proof in attestation aggregation third pass"
        ));
    }
