...
use validator_circuits::{circuits::wrappers::{bn128_wrapper_circuit_data_exists, bn128_wrapper_circuit_proof_exists, bn128_wrapper_clear_data_and_proof, load_or_create_bn128_wrapper_circuit, save_bn128_wrapper_proof}, circuits::{attestation_aggregation_circuit::AttestationAggregationCircuit, circuit_data_exists, circuit_init_proof_exists, clear_data_and_proof, load_or_create_circuit, load_or_create_init_proof, participation_state_circuit::ParticipationStateCircuit, validator_participation_circuit::ValidatorParticipationCircuit, validators_state_circuit::ValidatorsStateCircuit, Circuit, Proof, Serializeable, ATTESTATION_AGGREGATION_CIRCUIT_DIR, PARTICIPATION_STATE_CIRCUIT_DIR, VALIDATORS_STATE_CIRCUIT_DIR, VALIDATOR_PARTICIPATION_CIRCUIT_DIR}, circuits::wrappers::{create_groth16_wrapper_circuit, groth16_wrapper_circuit_data_exists, groth16_wrapper_circuit_proof_exists, groth16_wrapper_clear_data_and_proof}};
    ...
    if no_data || no_proof || no_bn128_wrapper_data || no_bn128_wrapper_proof {
        ...
        if no_bn128_wrapper_data || no_bn128_wrapper_proof {
            ...
            if no_bn128_wrapper_proof {
                ...
                let bn128_wrapper_proof = bn128_wrapper_circuit.generate_proof(circuit.circuit_data(), &example_proof.proof());
