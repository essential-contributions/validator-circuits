...
pub fn benchmark_validator_prove_participation(full: bool) {
    ...
    let (validators_tree, accounts_tree, validators_state_proof) = build_validators_state(
    ...
    let proof = validator_participation_circuit
        ...
        .unwrap();
    assert!(
        validator_participation_circuit.verify_proof(&proof).is_ok(),
    ...
    println!(
        "Proved validator participation at inputs hash 0x{}",
